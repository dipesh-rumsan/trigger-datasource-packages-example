...
describe('GlofasStationProcessorService', () => {
  ...
  beforeEach(async () => {
    ...
    service = module.get<GlofasStationProcessorService>(
      GlofasStationProcessorService,
    );
    ...
    jest
      ...
      .mockReturnValue('2023-01-02T10:05:00.000Z');
  ...
  describe('processGlofasStation', () => {
    ...
    it('should process Glofas station successfully', async () => {
      ...
      const result = await service.processGlofasStation(
        mockGlofasStation,
        errors,
      );
      ...
      // Verify data parsing
      expect(parseGlofasDataMock).toHaveBeenCalledWith(
        '<table>test reporting points data</table>',
      );
      ...
      expect(Logger.prototype.log).toHaveBeenCalledWith(
        'GLOFAS: Fetching data for Test River Basin on 2023-01-01',
      );
      expect(Logger.prototype.log).toHaveBeenCalledWith(
        'GLOFAS: Parsed data for Test River Basin on 2023-01-01',
      );
      expect(Logger.prototype.log).toHaveBeenCalledWith(
        'GLOFAS: Data saved successfully for Test River Basin on 2023-01-01',
      );
    });
    ...
    it('should handle errors during station data fetch', async () => {
      mockGlofasService.findGlofasDataByDate.mockResolvedValue(null);
      mockGlofasService.getStationData.mockRejectedValue(
        new Error('API error'),
      );
    ...
    it('should handle complex station data structure', async () => {
      ...
      expect(result).toBe(true);
      expect(parseGlofasDataMock).toHaveBeenCalledWith(
        '<table><tr><td>Complex</td><td>Data</td></tr></table>',
      );
      expect(mockSourceService.create).toHaveBeenCalledWith({
        source: 'GLOFAS',
        riverBasin: 'Test River Basin',
